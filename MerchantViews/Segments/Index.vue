    ...
    <div class="table_container">
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb bg-white m-0 p-0">
                        <li class="breadcrumb-item">
                            <router-link :to="{ name: 'segment.index' }">Segments</router-link>
                        </li>
                        <li class="breadcrumb-item active" aria-current="page">List Segments</li>
                    </ol>
                </nav>
            </div>
        </div>-->
      ...
      >
          ...
          <div>
            <div class="d-flex action-btn-main justify-content-center">
              <a
                class="mx-2 action-icon-btn"
                title="Review"
                v-on:click="redirectToReview(props.row['id'])"
              >
              ...
              <a
                class="mx-2 action-icon-btn"
                title="view"
                v-on:click="redirectToview(props.row['id'])"
              >
                <i class="fa fa-eye"></i>
              </a>
                            <a
                class="mx-2 action-icon-btn"
                title="Edit"
                v-on:click="redirectToEdit(props.row['id'])"
              >
                ...
                class="mx-2 action-icon-btn"
                title="Edit"
                v-on:click="redirectToCustomer(props.row['id'])"
              >
        ...
        <template slot="lastRefreshedDateSlot" slot-scope="props">
          <div>
            {{ getDateFormattedString(props.row["lastRefreshedDate"]) }}
          </div>
        </template>
...
<template slot="empty-results" v-if="isLoading"> Fetching records... </template>
           <template slot="empty-results" v-else> No record found! </template>      </DataTable>
    </div>
            <loading
...
export default {
  ...
  methods: {
    ...
    redirectToCus: (id) => {
      let path = `/merchantcustomer/?action=view&segment=${id}`;
    ...
    handleAddNew: function () {
      Vue.router.push({ path: `/merchantmanagesegments/?action=create` });
    },
    redirectToReview: function (id) {
      this.myDebug("Show customer for tier", id);
      let path = `/merchantreviewsegments/?action=view&segment=${id}`;
      Vue.router.push({ path: path });
    },
      ...
      //this.myDebug("Show customer for tier", id);
      let path = `/merchantmanagesegments/?action=edit&segment=${id}`;
      Vue.router.push({ path: path });
    },
  ...
  mounted() {
    ...
    this.$store
      .dispatch("merchantSegment/getAllSegmentList", {
