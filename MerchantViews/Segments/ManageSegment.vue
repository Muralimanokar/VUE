                     ...
                     <div class="row">
                         <div class="col-md-4">
                             <div class="form-group row" :class="{ 'form-group--error': $v.name.$error }">
                                ...
                                <div class="error col-md-12 text-left" style="margin-left:50%" v-if="$v.name.$error && !$v.name.required">Please enter segment name</div>
                                ...
                                <div class="error col-md-12 text-left" style="margin-left:50%" v-if="$v.name.$error && !$v.name.regex">Please enter a valid name</div>
                              </div>
                         </div>
                         <div class="col-md-4">
                             <div class="form-group row" :class="{ 'form-group--error': $v.description1.$error }">
                                ...
                                <div class="error col-md-12 text-left" style="margin-left:50%" v-if="$v.description1.$error && !$v.description1.maxLength">Maximum allowed length is 100 characters</div>
                              </div>
                         </div>
                     </div>
                     <div class="row">
                                ...
                                <div class="col-sm-6">
                                    <select class="form-control"
                                     ...
                                     :disabled="this.action =='view' "  @click="getrule()">
                                        <option value="SELECT" id="one" selected>Select</option>
                                        <option value="CUSTOMER" id="two">Customer</option>
                                       <!-- <option value="REWARD">Rewards</option>-->
                                        <option value="PURCHASE" id="three">Purchase</option>
                                    </select>
                                </div>
                                <div class="error col-md-12 text-left" style="margin-left:50%" v-if="$v.ruleCategory.$error && !$v.ruleCategory.required">Please select a rule category</div>
                              </div>
                         </div>
                          <div class="col-md-4">
           ...
           <div class="form-section" v-for="(item, idx) in rules" v-bind:key="item.id" >
             <div class="card">
                 ...
                 <div class="card-body px-0" >
                     <div class="row">
                        <!-- <div class="col-md-4">
                             <div class="form-group row" :class="{ 'form-group--error': $v.rules.$each[idx].level.$error }">
                                <label for="name" class="col-sm-6
                                    col-form-label text-right">Level</label>
                                <div class="col-sm-6">
                                   <input type="text" class="form-control"
                                     v-model="$v.rules.$each[idx].level.$model" value="">
                                </div>
                                <div class="error col-sm-12 text-right" v-if="$v.rules.$each[idx].level.$error && !$v.rules.$each[idx].level.required">Please enter level</div>
                              </div>
                        </div> -->
                         <div class="col-md-4">
                             <div class="form-group row" :class="{ 'form-group--error': $v.rules.$each[idx].field.$error }">
                                ...
                                <div class="col-sm-6" v-if="rules[idx].field==''">
                                    <select class="form-control"
                                ...
                                <div class="error col-md-12 text-left" style="margin-left:50%" v-if="$v.rules.$each[idx].field.$error && !$v.rules.$each[idx].field.required">Please select a field</div>
                              </div>
                         </div>
                     </div>
                     <div class="row" >
                        <!-- <div class="col-md-4" :class="{ 'form-group--error': $v.rules.$each[idx].aggregate.$error }">
                         ...
                         <div class="col-md-4" :class="{ 'form-group--error': $v.rules.$each[idx].ruleCondition.$error }" >
                             <div class="form-group row">
                                ...
                                <div class="error col-md-12 text-left" style="margin-left:50%" v-if="$v.rules.$each[idx].ruleCondition.$error && !$v.rules.$each[idx].ruleCondition.required">Please select a condition</div>
                              </div>
                         </div>
                           <div class="col-md-4">
                             <div class="form-group row" :class="{ 'form-group--error': $v.description2.$error }">
                                ...
                                <div class="error col-md-12 text-left" style="margin-left:50%" v-if="$v.description2.$error && !$v.description2.maxLength">Maximum allowed length is 100 characters</div>
                              </div>
                         </div>
                     </div>
                     <div class="row" >
                        <div class="col-md-4" >
                             <div class="form-group row" :class="{ 'form-group--error': $v.rules.$each[idx].value1.$error }">
                                ...
                                <div class="error col-md-12 text-left" style="margin-left:50%" v-if="$v.rules.$each[idx].value1.$error && !$v.rules.$each[idx].value1.required">Please enter a value</div>
                                <div class="error col-md-12 text-left" style="margin-left:50%" v-if="$v.rules.$each[idx].value1.$error && !$v.rules.$each[idx].value1.regex">Please enter the valid value</div>
                              </div>
                        </div>
                        <div class="col-md-4" v-if="rules[idx].ruleCondition=='BETWEEN'">
                             <div class="form-group row" :class="{ 'form-group--error': $v.rules.$each[idx].value2.$error }">
                                ...
                                <div class="error col-md-12 text-left" style="margin-left:50%" v-if="$v.rules.$each[idx].value2.$error && !$v.rules.$each[idx].value2.required">Please enter a value</div>
                              </div>
                        </div>
                     </div>
                     <div class="row">
                        ...
                        <!-- <div class="col-md-4">
                         ...
                         <div class="col-md-4">
                             <div class="form-group row" :class="{ 'form-group--error': $v.rules.$each[idx].field.$error }">
                                ...
                                <div class="col-sm-6" v-else :disabled="true">
                                   <label class="font-weight-bold" :disabled="true" > {{ getFieldText(rules[idx].field) }} </label>
                                   <a class="btn icon-btn btn-sm ml-2 bg-light disabled" @click="editField(idx)">
                     ...
                     <div class="row">
                         <div class="col-md-4">
                             <div class="form-group row" :class="{ 'form-group--error': $v.rules.$each[idx].isActive.$error }">
                                ...
                                <div class="col-sm-6">
                                     <div class="custom-control custom-switch">
                                       <input type="checkbox" :id="item.id"   :disabled="true" class="custom-control-input"
                                         v-model="$v.rules.$each[idx].isActive.$model">
                                       <label :for="item.id" class="custom-control-label"></label>
                                     </div>
                                </div>
...
import { required, requiredIf, maxLength, helpers, regex } from "vuelidate/lib/validators";
 ...
 this.fieldOpts=[];
    ...
    handleSubmit() {
      if(this.isFormValid()) {
        ...
        console.log("check the dataa",postJson);
        let storeAction = "";
        let message="";
        if(this.action == "create") {
          storeAction = "merchantSegment/create";
          message="Segment created successfully";
        } else if(this.action == "edit") {
          console.log("edit section  data");
          postJson.data.id = this.id;
          message="Segment updated successfully";
          storeAction = "merchantSegment/update";
        } else {
        ...
        this.$store.dispatch(storeAction, postJson).then(function(res) {
          ...
          cachescope.isLoading = false;
          cachescope.$router.push({ path: `/merchantsegments` });
        }).catch(function(err){
          let msg=err.message;
          cachescope.submitStatus = "IDLE";
          cachescope.$toast.open({
    ...
    handleCancel() {
      this.$router.push({ path: `/merchantsegments` });
    },
    isFormValid() {
      this.$v.$touch();
      if (!this.$v.$invalid) {
