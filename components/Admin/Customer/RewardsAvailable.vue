<template>
  <div>
    <div class="card-header" style="background: #34b8df;">
      <h2 class="mb-0" style="font-size:13px;">Rewards Available</h2>
    </div>
    <div class="table_container">
       <DataTable :rows="rows" :columns="columns" :config="config" :dtName="'earnList'">
            ...
            <template slot="expieryDateSlot"  slot-scope="props">
               <div>{{ getDateFormattedString(props.row["expiryDate"]) }}</div>
            </template>
...
export default {
  ...
  methods: {
    getCustomerRewardsAvailable(id) {
      let cachescope = this;
  ...
  partnerId:this.partnerId
      ...
      this.$store.dispatch("customer/getRewardsAvailable",dataobj).then(function(res) {
    ...
    handleViewItem(id) {
      ...
      let path = `/manageReward/?action=view&reward=${this.customerId}`;
  ...
  mounted() {
    ...
    */
    ...
    if(this.customerId != undefined) {
      this.getCustomerRewardsAvailable(this.customerId);
    }
  }
};
</script>
